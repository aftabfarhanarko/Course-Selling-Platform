...
const AI_RESPONSES = {
  "Is this for beginners?": "This course suits intermediate designers. Basic Figma/design knowledge is recommended, but beginners with strong motivation do just fine! 💪",
  "What do I get?": "You get 40+ hours of video, live project templates, a private community, lifetime access, and 1-on-1 feedback sessions. 🎯",
  "Is there a refund policy?": "Yes! Full 30-day money-back guarantee — no questions asked. Your investment is risk-free. ✅",
};
...
function AIPanel({ open }) {
  const [messages, setMessages] = useState([
    { type: "ai", text: "Hi! 👋 I'm your AI guide for this masterclass. What would you like to know?" },
  ]);
  const [chips, setChips] = useState(["Is this for beginners?", "What do I get?", "Is there a refund policy?"]);
  ...
  const sendChip = (chip) => {
    ...
    addReply(AI_RESPONSES[chip] || GENERIC[Math.floor(Math.random() * GENERIC.length)]);
  };
  ...
  const sendMessage = () => {
    ...
    addReply(AI_RESPONSES[val] || GENERIC[Math.floor(Math.random() * GENERIC.length)]);
  };
    ...
    >
      ...
      <div className="bg-[#1a4bff] px-4 py-3 flex items-center gap-3">
        ...
        <div className="text-white flex-1">
          ...
          <div className="text-[11px] opacity-75">Ask anything about the course</div>
        </div>
        <div className="bg-white/20 text-white text-[10px] font-bold px-2 py-0.5 rounded-full tracking-wide">AI</div>
      </div>

      {/* Messages */}
      <div ref={msgRef} className="px-3.5 pt-3.5 flex flex-col gap-2.5 max-h-[200px] overflow-y-auto">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-2 items-start ${msg.type === "user" ? "flex-row-reverse" : ""}`}>
            <div
        ...
        {typing && (
          <div className="flex gap-2 items-start">
            <div className="w-[26px] h-[26px] rounded-full bg-gradient-to-br from-[#c7d6ff] to-[#a5b4fc] flex-shrink-0 flex items-center justify-center text-[11px] font-bold text-[#1a4bff]">✦</div>
      ...
      <div className="px-3.5 pb-3.5 pt-2 flex gap-2 items-center border-t border-[#f0f3fa]">
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <polygon points="22 2 15 22 11 13 2 9 22 2" fill="rgba(255,255,255,0.3)" stroke="white" />
          </svg>
        </button>
      </div>
    </div>
      ...
      >
        ...
        <div className="flex flex-col gap-6">
          ...
          <h1 className="font-['Syne'] text-[clamp(2.2rem,4.8vw,3.5rem)] font-extrabold leading-[1.07] tracking-[-0.022em] text-[#0c1a3b]">
            SaaS Interface{" "}
            <em className="not-italic text-[#1a4bff]">Architect</em>{" "}
            Masterclass
          </h1>

          <p className="text-base leading-relaxed text-[#5b6580] max-w-md">
            Master the high-ticket skill of SaaS product design. Move beyond "UI design" and start building scalable interface architectures that command $10k+ project fees.
          </p>
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium text-[#0c1a3b] text-sm">Alex Rivera</div>
            ...
            <div className="w-px h-8 bg-[#d0d5e8] hidden sm:block" />
            <div className="text-sm font-medium text-[#0c1a3b]">5,000+ Students</div>
          </div>
          ...
          <div className="flex items-center gap-5 flex-wrap">
            ...
            <div className="text-sm text-[#5b6580]">
              Limited time <span className="line-through text-gray-400">50% discount</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex justify-center">
          ...
          <div className="w-full max-w-[500px] bg-white rounded-3xl shadow-[0_32px_80px_rgba(12,26,59,0.14)] overflow-hidden">
            ...
            <div className="relative bg-[#f8f9fc] p-5">
              ...
              <div className="text-[#8896b3] text-[11px] -mt-1 mb-4">Load your interface, save time</div>
              ...
              <div className="absolute right-0 top-0 bottom-0 w-[42%] bg-gradient-to-br from-[#7db5e8] via-[#a8d5f0] to-[#b8e0f0] rounded-tl-[60px] overflow-hidden">
                <svg className="absolute bottom-0 left-0 right-0 w-full" viewBox="0 0 200 120" preserveAspectRatio="xMidYMax slice">
                  <path d="M0 120 L60 40 L100 70 L140 20 L200 80 L200 120 Z" fill="rgba(40,90,50,0.6)" />
                  <path d="M0 120 L80 60 L120 90 L160 50 L200 100 L200 120 Z" fill="rgba(30,70,40,0.5)" />
                </svg>
              </div>
            </div>
          </div>
          ...
          <div className="absolute -bottom-5 -left-6 bg-white rounded-2xl p-5 shadow-[0_16px_40px_rgba(12,26,59,0.16)] min-w-[180px] animate-[float_3s_ease-in-out_infinite]">
            <div className="text-[#1a4bff] text-[10px] font-bold tracking-widest uppercase mb-1">Student Revenue</div>
            <div className="font-['Syne'] text-3xl font-extrabold text-[#0c1a3b]">$8,500/mo</div>
            <div className="text-[#22c55e] text-sm flex items-center gap-1 mt-2">↗ Avg. Project Target</div>
          </div>
        </div>
      </div>
    </div>

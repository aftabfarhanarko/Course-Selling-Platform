...
type UiProduct = {
  ...
  status: string;
  createdAt: string;
  // structured extra fields extracted from raw
  description: string;
  botName: string;
  countryCodes: string[];
...
function extractTotal(payload: any): number | null {
  const candidates = [
    payload?.meta?.total, payload?.data?.meta?.total,
    payload?.pagination?.total, payload?.data?.pagination?.total,
...
const KNOWN_KEYS = new Set([
  "id", "_id", "productId", "courseId",
  "botName", "title", "name", "productName",
  "category", "categoryName",
  "totalAmount", "price", "amount", "total",
...
function toUi(raw: any): UiProduct | null {
  ...
  const title = String(raw?.botName ?? raw?.title ?? raw?.name ?? raw?.productName ?? raw?.course?.title ?? "—").trim() || "—";
  const category = String(raw?.category ?? raw?.categoryName ?? raw?.course?.category ?? "—").trim() || "—";
  const priceRaw = raw?.totalAmount ?? raw?.price ?? raw?.amount ?? raw?.total ?? null;
  const price = priceRaw == null || priceRaw === "" ? "—" : String(priceRaw);
  const status = String(raw?.status ?? raw?.state ?? raw?.approvalStatus ?? "—").trim() || "—";
  ...
  const countryCodes: string[] = Array.isArray(raw?.countryCodes) ? raw.countryCodes : [];
  ...
  return { id, title, category, price, status, createdAt, description, botName, countryCodes, extra };
}
...
const isApproved = (s: string) => ["approved", "active", "paid"].includes(s.toLowerCase());
...
function StatCard({ icon: Icon, label, value, color }: { icon: React.FC<any>; label: string; value: string | number; color: string }) {
  return (
...
function Field({ label, value, icon: Icon, mono }: { label: string; value: string; icon?: React.FC<any>; mono?: boolean }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-gray-50/60 px-4 py-3">
      ...
      <p className={`text-[13px] font-medium text-gray-900 break-words ${mono ? "font-mono text-[11px]" : ""}`}>{value}</p>
    </div>
  );
}

/* ─── Details Modal ──────────────────────────────────────────────── */
function ProductDetailsModal({ product, onClose, onWithdraw, isWithdrawing }: {
  product: UiProduct;
  onClose: () => void;
  onWithdraw: () => void;
        ...
        <div className="px-5 sm:px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/60 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-[14px] font-extrabold text-gray-900 truncate">Product Details</h2>
              <p className="text-[11px] text-gray-400 mt-0.5 truncate">{product.title}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-400 hover:bg-gray-200 hover:text-gray-700 transition-colors flex-shrink-0">
            <X size={15} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 p-5 text-white">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-widest text-indigo-200 mb-1">Product</p>
                <h3 className="text-xl font-black break-words">{product.title}</h3>
                {product.category !== "—" && (
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="bg-white/15 rounded-xl p-3">
                <p className="text-[10px] text-indigo-200 uppercase tracking-widest font-bold mb-1">Price</p>
                <p className="text-2xl font-black">
                  {product.price !== "—" ? `$${Number(product.price).toLocaleString()}` : "—"}
                </p>
              </div>
              <div className="bg-white/15 rounded-xl p-3">
                <p className="text-[10px] text-indigo-200 uppercase tracking-widest font-bold mb-1">Created</p>
          ...
          <div className="grid grid-cols-1 gap-3">
            <Field label="Product ID"    value={String(product.id)}    icon={Hash}       mono />
            ...
            <Field label="Price"         value={product.price !== "—" ? `$${Number(product.price).toLocaleString()}` : "—"} icon={DollarSign} />
            <Field label="Created At"    value={product.createdAt}      icon={Calendar} />
            {product.description && (
              <Field label="Description" value={product.description}    icon={Package} />
            )}
          </div>
          ...
          {product.countryCodes.length > 0 && (
            <div className="rounded-2xl border border-gray-100 bg-gray-50/60 px-4 py-3">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider">Target Countries</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {product.countryCodes.map((c) => (
                  <span key={c} className="bg-white border border-gray-200 text-gray-700 px-3 py-1 rounded-xl text-[11px] font-bold shadow-sm flex items-center gap-1">
          ...
          {/* Extra fields from API (shown as labeled text, never raw JSON dump) */}
          {product.extra.map(({ label, value }) => (
            <div key={label} className="rounded-2xl border border-gray-100 bg-gray-50/60 px-4 py-3">
              ...
              <p className="text-[12px] font-medium text-gray-800 break-words whitespace-pre-wrap">{value}</p>
            </div>
          ))}
          ...
          {isApproved(product.status) && (
            ...
            >
              {isWithdrawing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Banknote className="w-4 h-4" />}
              Request Withdrawal
            </button>
          )}
        </div>
...
/* ─── Mobile card ────────────────────────────────────────────────── */
function MobileProductCard({ p, onView, onWithdraw, isWithdrawing }: {
  p: UiProduct;
  onView: () => void;
  onWithdraw: () => void;
  isWithdrawing: boolean;
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-100 to-violet-100 flex items-center justify-center flex-shrink-0">
            <span className="text-[11px] font-black text-indigo-600">{p.title.charAt(0).toUpperCase()}</span>
          </div>
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap mt-1">
              ...
              {p.category !== "—" && (
                <span className="text-[11px] text-violet-700 bg-violet-50 border border-violet-100 px-2 py-0.5 rounded-lg font-bold">{p.category}</span>
              )}
            </div>
...
export default function StudentProductsManager() {
  ...
  const totalPages = total === null ? Math.max(1, page) : Math.max(1, Math.ceil(total / PAGE_SIZE));
  const canPrev    = page > 1;
  const canNext    = page < totalPages;

  const approvedCount = items.filter(p => isApproved(p.status)).length;
  const pendingCount  = items.filter(p => p.status.toLowerCase() === "pending").length;
  const totalValue    = items.reduce((s, p) => s + (isNaN(Number(p.price)) ? 0 : Number(p.price)), 0);
  ...
  return (
    <div className="min-h-screen pb-16">
      ...
      <div className="border-b border-gray-200 px-4 sm:px-8 py-6">
        <div className="w-full max-w-[1400px] mx-auto">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-black uppercase tracking-[0.18em] text-indigo-500 bg-indigo-50 border border-indigo-100 rounded-full px-2.5 py-0.5">Student</span>
                </div>
                <h1 className="mt-1 text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">My Products</h1>
          ...
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatCard icon={Package}    label="Total Products" value={total ?? items.length}          color="bg-indigo-100 text-indigo-600" />
            <StatCard icon={CheckCircle2} label="Approved"     value={approvedCount}                  color="bg-emerald-100 text-emerald-600" />
            <StatCard icon={HourglassIcon} label="Pending"     value={pendingCount}                   color="bg-amber-100 text-amber-600" />
            <StatCard icon={TrendingUp}  label="Total Value"   value={`$${totalValue.toLocaleString()}`} color="bg-violet-100 text-violet-600" />
          </div>
        </div>
      </div>
          ...
          <div className="px-4 sm:px-5 py-4 border-b border-gray-100 bg-gray-50/40">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex items-center justify-between sm:justify-end gap-2">
                ...
                >
                  <ChevronLeft className="h-3.5 w-3.5" /> <span className="hidden xs:inline">Prev</span>
                </button>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-xl border border-gray-200 bg-white px-3 py-2 text-[12px] font-bold text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
                >
                  <span className="hidden xs:inline">Next</span> <ChevronRight className="h-3.5 w-3.5" />
                </button>
              </div>
            </div>
          </div>
          ...
          {isFetching ? (
            ...
            <div className="flex items-center justify-center py-20">
              <div className="inline-flex flex-col items-center gap-3">
                ...
                <p className="text-[12px] font-bold text-red-600">Failed to load products</p>
                <button onClick={() => refetch?.()} className="text-[11px] font-bold text-indigo-600 hover:underline">Try again</button>
              </div>
            </div>
          ) : items.length === 0 ? (
            <div className="flex items-center justify-center py-20">
              <div className="inline-flex flex-col items-center gap-3 text-gray-500">
                ...
                <p className="text-[13px] font-bold text-gray-700">No products found</p>
                <button
                  onClick={() => setCreate(true)}
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full w-full">
                  <thead>
                    <tr className="bg-gray-50/80 border-b border-gray-100">
                      {COLUMNS.map(col => {
                        const Icon = col.icon;
                        return (
                          <th key={col.key} className="px-5 py-3.5 text-left whitespace-nowrap text-xs font-semibold text-gray-700">
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-lg bg-white border border-gray-200 flex items-center justify-center shadow-sm">
                                <Icon className="w-3.5 h-3.5 text-indigo-500" />
                              </div>
                              <div>
                                <p className="uppercase tracking-wider">{col.label}</p>
                                <p className="text-[9px] text-gray-400 font-semibold leading-none mt-0.5">{col.sub}</p>
                              </div>
                            </div>
                          </th>
                        );
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {items.map(p => (
                      <tr key={String(p.id)} className="hover:bg-indigo-50/30 transition-colors group">
                        {/* Product */}
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="text-[13px] font-bold text-gray-900 truncate max-w-[180px]">{p.title}</p>
                              <p className="text-[10px] font-mono text-gray-400 mt-0.5 flex items-center gap-1">
                                <Hash className="w-2.5 h-2.5" />
                                {String(p.id).slice(0, 12)}{String(p.id).length > 12 ? "…" : ""}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[12px] font-semibold text-gray-600 whitespace-nowrap">{p.createdAt}</span>
                          </div>
                        </td>
                        {/* Actions */}
                        <td className="px-5 py-4">
          ...
          {items.length > 0 && (
            <div className="px-4 sm:px-5 py-3 border-t border-gray-100 bg-gray-50/40 flex flex-col sm:flex-row items-center justify-between gap-3">
              <p className="text-[11px] font-semibold text-gray-400">
                Showing <span className="font-bold text-gray-600">{items.length}</span> of{" "}
              ...
              <div className="flex items-center gap-1.5">
                ...
                {totalPages > 5 && <span className="text-gray-400 text-[11px]">…</span>}
              </div>
            </div>
          )}
        </div>
      ...
      <CreateProductModal isOpen={isCreateOpen} onClose={() => setCreate(false)} />
    </div>
  );
}

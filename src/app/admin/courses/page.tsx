// app/admin/courses/page.tsx (or wherever you place the page)
"use client";
...
} from "@/lib/api/admin/courses"; // your provided course API
import {
  useAdminCategoriesQuery, // comes from your existing category API
...
/* ---- Data extraction / normalisation (same idea as your category page) ---- */
function extractCourses(payload: any): any[] {
  if (!payload) return [];
...
/* ------------------------------------------------------------------ */
/*  Modal shells (identical to your category page)                     */
/* ------------------------------------------------------------------ */
function ModalShell({
  title,
  subtitle,
...
/* ------------------------------------------------------------------ */
/*  Course Form Modal (with category dropdown)                         */
/* ------------------------------------------------------------------ */
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* Category selector – mandatory before publishing */}
        <div>
...
/* ------------------------------------------------------------------ */
/*  View details modal (using lazy query)                              */
/* ------------------------------------------------------------------ */
function ViewModal({
  id,
  open,
...
export default function AdminCoursesPage(): React.JSX.Element {
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter((c) => {
      const hay =
        `${c.name} ${c.description ?? ""} ${c.categoryName}`.toLowerCase();
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70">
                  {["Name", "Category", "Created", "Status", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
